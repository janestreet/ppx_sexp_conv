...
module Exc_test : sig
  exception Test_exc of (string * int) [@@deriving sexp]
end = struct
  exception Test_exc of (string * int) [@@deriving sexp]
end
...
type empty [@@deriving sexp]

type 'a function_field_with_labeled_argument = { f : x:'a -> 'a } [@@deriving sexp]

(* Test non-regular types *)
type 'a nonregular = Leaf of 'a | Branch of ('a * 'a) nonregular [@@deriving sexp]
...
type labeled = string -> foo : unit -> ?bar : int -> float -> float [@@deriving sexp]
...
type rec_labeled = { a : (foo : unit -> unit) } [@@deriving sexp_of]
...
and 'a t =
  {
    ...
    default_1 : int [@default 1] [@sexp_drop_default];
    default_2 : int [@default 2] [@sexp_drop_if (=) 2];
...
(* Test function types *)
type fun_test = int -> unit [@@deriving sexp_of]

open Path

type does_sexp_array_type_check =
  { does_sexp_array_type_check_field : string sexp_array; } [@@deriving sexp]
