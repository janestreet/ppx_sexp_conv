...
module M : sig
  type ('a[@phantom], 'b) t = 'b [@@deriving_inline sexp]

  val sexp_of_t : 'a 'b. ('b -> Sexplib0.Sexp.t) -> ('a, 'b) t -> Sexplib0.Sexp.t
  val t_of_sexp : 'a 'b. (Sexplib0.Sexp.t -> 'b) -> Sexplib0.Sexp.t -> ('a, 'b) t

  [@@@end]

  type ('a[@phantom], 'b) alias = ('a, 'b) t [@@deriving_inline sexp]

  val sexp_of_alias : 'a 'b. ('b -> Sexplib0.Sexp.t) -> ('a, 'b) alias -> Sexplib0.Sexp.t
  val alias_of_sexp : 'a 'b. (Sexplib0.Sexp.t -> 'b) -> Sexplib0.Sexp.t -> ('a, 'b) alias
  ...
  type ('a[@phantom], 'b) alias = (('a[@phantom]), 'b) t [@@deriving_inline sexp]

  let alias_of_sexp : 'b 'a. (Sexplib0.Sexp.t -> 'b) -> Sexplib0.Sexp.t -> ('a, 'b) alias =
    t_of_sexp
  ;;

  let sexp_of_alias : 'b 'a. ('b -> Sexplib0.Sexp.t) -> ('a, 'b) alias -> Sexplib0.Sexp.t =
    sexp_of_t
  ;;
