...
module Str_generate_sexp_of = struct
  ...
  let rec sexp_of_type
    ...
    | [%type: _] -> Fun [%expr fun _ -> Ppx_sexp_conv_lib.Sexp.Atom "_"]
    | [%type: [%t? _] sexp_opaque] -> Fun [%expr Ppx_sexp_conv_lib.Conv.sexp_of_opaque]
    | { ptyp_desc = Ptyp_tuple tp; _ } ->
      Match [ sexp_of_tuple ~typevar_handling (loc, tp) ]
      ...
      Fun
        (sexp_of_type_constr
           ...
           (List.map args ~f:(fun tp ->
              Fun_or_match.expr ~loc (sexp_of_type ~typevar_handling tp))))
  ...
  and sexp_of_variant ~typevar_handling ((loc, row_fields) : Location.t * row_field list)
    ...
    let item row =
      ...
      | Rtag ({ txt = cnstr; _ }, false, [ tp ]) ->
        ...
        let cnstr_arg =
          Fun_or_match.unroll ~loc var (sexp_of_type ~typevar_handling tp)
        in
        let expr =
          [%expr Ppx_sexp_conv_lib.Sexp.List [%e elist ~loc [ cnstr_expr; cnstr_arg ]]]
        in
            ...
            | Inspect_value is_empty_expr ->
              [%expr
                ...
                then bnds
                  ...
                  let bnd =
                    Ppx_sexp_conv_lib.Sexp.List
                      [ Ppx_sexp_conv_lib.Sexp.Atom [%e estring ~loc name]; arg ]
                  in
                  bnd :: bnds)]
    ...
    let is_empty =
      ...
      | (`no_arg | `func _ | `compare | `equal) as how ->
        let equality_f loc =
          ...
          | `no_arg ->
            [%expr
              Ppx_sexp_conv_lib.Conv.( = ) [@ocaml.ppwarning
                "[@sexp_drop_default] is deprecated: please \
                 use one of:\n\
                 - [@sexp_drop_default f] and give an \
                 explicit equality function ([f = Poly.(=)] \
          ...
          [%expr
            let bnds =
              ...
              then (
                let bnd =
                  Ppx_sexp_conv_lib.Sexp.List
                    [ Ppx_sexp_conv_lib.Sexp.Atom [%e estring ~loc name] ]
                in
                bnd :: bnds)
              else bnds
            in
    ...
    | Pcstr_tuple pcd_args ->
      (match pcd_args with
       | [] ->
         ...
         --> [%expr Ppx_sexp_conv_lib.Sexp.Atom [%e constr_str]]
       | args ->
         (match args with
          | [ tp ] when Option.is_some (Attribute.get inline_attr row) ->
            (match tp with
             | [%type: [%t? tp] list] ->
               let cnv_expr =
                 Fun_or_match.expr ~loc (sexp_of_type ~typevar_handling:(`ok renaming) tp)
               in
        ...
        | Ptype_record lds ->
          ...
          let patt, expr =
            sexp_of_label_declaration_list
              ...
              lds
              ~types_being_defined
              ~wrap_expr:(fun expr -> [%expr Ppx_sexp_conv_lib.Sexp.List [%e expr]])
          in
          Match [ patt --> expr ]
        | Ptype_open ->
...
module Str_generate_of_sexp = struct
  ...
  let mk_variant_other_matches loc rev_els call =
    let coll_structs acc (loc, cnstr) =
      (pstring ~loc cnstr
       ...
       | `ptag_takes_args ->
         [%expr Ppx_sexp_conv_lib.Conv_error.ptag_takes_args _tp_loc _sexp])
      :: acc
    in
  ...
  (* Conversion of types *)
  let rec type_of_sexp ~typevar_handling ?full_type ?(internal = false) typ
    : Fun_or_match.t
    =
    ...
    | [%type: [%t? ty1] sexp_list] ->
      ...
      Fun
        [%expr
          (fun a_of_sexp v -> Ppx_sexp_conv_lib.Conv.list_of_sexp a_of_sexp v) [%e arg1]]
    | [%type: [%t? ty1] sexp_array] ->
      let arg1 = Fun_or_match.expr ~loc (type_of_sexp ~typevar_handling ty1) in
      Fun
        [%expr
          (fun a_of_sexp v -> Ppx_sexp_conv_lib.Conv.array_of_sexp a_of_sexp v) [%e arg1]]
    | { ptyp_desc = Ptyp_tuple tp; _ } ->
      Match (tuple_of_sexp ~typevar_handling (loc, tp))
    | { ptyp_desc = Ptyp_var parm; _ } ->
      (match typevar_handling with
    ...
    | { ptyp_desc = Ptyp_constr (id, args); _ } ->
      let args =
        List.map args ~f:(fun arg ->
          Fun_or_match.expr ~loc (type_of_sexp ~typevar_handling arg))
      in
      Fun (type_constr_of_sexp ~loc ~internal id args)
    | { ptyp_desc = Ptyp_arrow (_, _, _); _ } ->
      Fun [%expr Ppx_sexp_conv_lib.Conv.fun_of_sexp]
    | { ptyp_desc = Ptyp_variant (row_fields, Closed, _); _ } ->
        ...
        | _ ->
          [%e
            ...
            then
              ...
              [%expr
                Ppx_sexp_conv_lib.Conv_error.stag_incorrect_n_args _tp_loc _tag _sexp]]]
             ...
             (pstring ~loc nm
              --> [%expr
                ...
                then duplicates := field_name :: !duplicates
                  ...
                  | _ :: _ ->
                    Ppx_sexp_conv_lib.Conv_error.record_sexp_bool_with_payload
                      _tp_loc
                      sexp)])
             :: cases
           in
        ...
        | ({ pld_name = { txt = nm; loc }; _ } as ld) :: more_flds ->
          ...
          let new_bi_lst, new_good_patts =
            ...
            | None ->
              ...
              ( [%expr
                Ppx_sexp_conv_lib.Conv.( = ) [%e fld] Ppx_sexp_conv_lib.Option.None
              , [%e estring ~loc nm]]
                :: bi_lst
              , [%pat? Ppx_sexp_conv_lib.Option.Some [%p pvar ~loc (nm ^ "_value")]]
                :: good_patts )
          in
          ...
          | Ppx_sexp_conv_lib.Option.None ->
            ...
            (try
               [%e
                 Fun_or_match.unroll
                   ~loc
                   [%expr Ppx_sexp_conv_lib.Sexp.List []]
                   (type_of_sexp ~typevar_handling ld.pld_type)]
             with
             | Ppx_sexp_conv_lib.Conv_error.Of_sexp_error (e, _sexp) ->
  ...
  let mk_good_sum_matches ~typevar_handling (loc, cds) =
    List.map cds ~f:(fun cd ->
      ...
      | { pcd_name = cnstr; pcd_args = Pcstr_tuple (_ :: _ as tps); _ } ->
        ...
        [%pat?
               Ppx_sexp_conv_lib.Sexp.List
               (Ppx_sexp_conv_lib.Sexp.Atom (([%p lcstr] | [%p str]) as _tag) :: sexp_args)
          as _sexp]
        --> mk_cnstr_args_match
              ~typevar_handling
              ~loc
...
module Of_sexp = struct
  ...
  let core_type =
    Str_generate_of_sexp.type_of_sexp ~typevar_handling:`disallowed_in_type_expr
  ;;
...
module Sig_sexp = struct
  ...
  let sig_type_decl ~loc ~path ((_rf, tds) as decls) =
    match
      mk_named_sig
        ~loc
        ~sg_name:"Ppx_sexp_conv_lib.Sexpable.S"
